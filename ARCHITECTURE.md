...
Grouped by project below; numbering stays flat (1-21) across groups so existing `ARCHITECTURE.md §N` cross-references from code and READMEs stay valid.

## Common decisions (1-9)
...
- **Two functions ported, not imported**: `normalize_radar()` (10 lines of pure pandas) and `lexical.py`'s regex tokenizer (3 lines) are copied into `data_access/style_duel.py` with an attribution note, rather than depending on the full package for about 15 lines.
...
### 10. The 13 topic labels are new editorial work
...
### 11. The Covid zoom's rationale genuinely exists, in `app.py`, not in a report
...
### 12. Tenure dates read from `hansard-pm-nlp`, not from a third cloned repo
...
### 13. Cividis scaled 0 to max, not to a percentile
...
### 14. Built in this repo, from a roadmap, not a design spec
...
### 15. `vader_compound` replaces MTLD as the second "star" metric
...
### 16. `event_study_dataset.parquet` as the single data source, not Phase 3/4's own exports
...
### 17. A fixed +/- 6 week window surfaces a real, unanticipated gap
...
### 18. A design document written before the rendering code, per the roadmap's own recommendation
...
### 19. Built on `event_study_dataset.parquet` + `lda_topics.parquet` alone, not the roadmap's suggested 4 layers
...
### 20. 2026 is a second partial year, not anticipated in the roadmap
...
### 21. `data_access.py` split into a package, one file per project plus `_shared.py`

By project 04, `data_access.py` had grown to 537 lines and 25 functions across all 4 projects, flagged in a repo audit as a maintainability problem: a reader auditing one project had to scan the whole file. Split into `data_access/` (a package): `_shared.py` holds infrastructure 2+ projects reuse (I/O, PM scope, tenure dates, crisis windows, and the LDA topic-weight layer projects 02 and 04 both build on), and `style_duel.py` / `topic_heatmap.py` / `pm_handover.py` / `annual_recap.py` hold each project's own logic only. Largest file after the split: `_shared.py` at 190 lines, versus 537 before.

`__init__.py` re-exports the full combined public API (`__all__`, 33 names), so every existing call site (`from hansard_pm_portfolio import data_access as da; da.load_style_profile()`, used throughout `viz/`, `tests/`, and all 4 notebooks) needed zero changes - the package is a drop-in replacement for the flat file, not a breaking rename. The trade-off: a reader of `viz/annual_recap.py` still sees `da.something()` rather than an explicit `from data_access.annual_recap import ...`, so the module boundary is visible in `data_access/` itself but not enforced at every call site. Chosen deliberately over touching 9+ already-tested files' imports for a smaller, secondary benefit - see the audit's own recommendation for the reasoning.

Two tests broke in the split and were fixed, not worked around: `_function_word_rate()` is project 01-only and intentionally not re-exported (it's a private helper, not public API), so its test now imports it directly from `data_access.style_duel`. A `monkeypatch.setattr(da, "load_pm_tenures", ...)` test for `load_pm_transitions()` silently stopped intercepting the call once the two functions moved to different files (`load_pm_transitions` resolves `load_pm_tenures` via `pm_handover.py`'s own module namespace, not the package's re-exported name) - fixed by patching `da.pm_handover.load_pm_tenures` instead.
